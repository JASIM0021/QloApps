		...
		{if isset($css_files)}
			{foreach from=$css_files key=css_uri item=media}
				{if $css_uri == 'lteIE9'}
					...
					<link rel="stylesheet" href="{$css_uriie9|escape:'html':'UTF-8'}{if strpos($css_uriie9, '?') === false}?v={$smarty.now}{else}&v={$smarty.now}{/if}" type="text/css" media="{$mediaie9|escape:'html':'UTF-8'}" />
					{/foreach}
					<![endif]-->
				{else}
